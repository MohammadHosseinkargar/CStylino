        ...
        <div className="lg:sticky lg:top-24 h-fit order-first lg:order-last">
          <GlassCard className="p-6">
            <div className="pb-4 md:pb-6">
              <div className="text-base md:text-title font-semibold">{fa.checkout.orderSummary}</div>
            </div>
            <div className="space-y-5 sm:space-y-6">
                ...
                {items.map((item) => (
                  <div key={item.variantId} className="flex justify-between text-body">
                    <span className="text-muted-foreground line-clamp-1">
                      {item.productName} - <span dir="ltr">{item.quantity}</span> {fa.checkout.itemCountSuffix}
                    </span>
